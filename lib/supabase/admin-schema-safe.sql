-- Admin functionality database schema extensions (SAFE VERSION)
-- Handles missing tables gracefully

-- 1. User Audit Logs Table
CREATE TABLE IF NOT EXISTS user_audit_logs (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id TEXT NOT NULL REFERENCES users(id),
    admin_id TEXT NOT NULL REFERENCES users(id),
    action VARCHAR(50) NOT NULL,
    table_name VARCHAR(100) NOT NULL,
    record_id TEXT NOT NULL,
    old_data JSONB,
    new_data JSONB,
    changes JSONB,
    ip_address VARCHAR(45),
    user_agent TEXT,
    metadata JSONB,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_user_audit_logs_user_id ON user_audit_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_user_audit_logs_admin_id ON user_audit_logs(admin_id);
CREATE INDEX IF NOT EXISTS idx_user_audit_logs_action ON user_audit_logs(action);
CREATE INDEX IF NOT EXISTS idx_user_audit_logs_created_at ON user_audit_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_audit_logs_table_record ON user_audit_logs(table_name, record_id);

-- 2. User Versions Table
CREATE TABLE IF NOT EXISTS user_versions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id TEXT NOT NULL REFERENCES users(id),
    version_number INTEGER NOT NULL,
    email VARCHAR(255) NOT NULL,
    first_name VARCHAR(100),
    last_name VARCHAR(100),
    role VARCHAR(20) NOT NULL,
    is_active BOOLEAN NOT NULL DEFAULT true,
    image_url TEXT,
    public_metadata JSONB,
    private_metadata JSONB,
    created_by TEXT REFERENCES users(id),
    created_at TIMESTAMPTZ NOT NULL,
    valid_from TIMESTAMPTZ NOT NULL,
    valid_to TIMESTAMPTZ,
    change_reason TEXT,
    UNIQUE(user_id, version_number)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_user_versions_user_id ON user_versions(user_id);
CREATE INDEX IF NOT EXISTS idx_user_versions_valid_period ON user_versions(user_id, valid_from, valid_to);

-- 3. Candidate Profile Versions Table
CREATE TABLE IF NOT EXISTS candidate_profile_versions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    profile_id UUID NOT NULL REFERENCES candidate_profiles(id),
    user_id TEXT NOT NULL REFERENCES users(id),
    version_number INTEGER NOT NULL,
    title VARCHAR(255),
    summary TEXT,
    experience_level VARCHAR(20),
    location VARCHAR(100),
    remote_preference VARCHAR(20),
    availability VARCHAR(20),
    salary_min INTEGER,
    salary_max INTEGER,
    salary_currency VARCHAR(3),
    linkedin_url TEXT,
    github_url TEXT,
    portfolio_url TEXT,
    resume_url TEXT,
    is_anonymized BOOLEAN DEFAULT false,
    is_active BOOLEAN DEFAULT true,
    public_metadata JSONB,
    private_metadata JSONB,
    created_by TEXT REFERENCES users(id),
    created_at TIMESTAMPTZ NOT NULL,
    valid_from TIMESTAMPTZ NOT NULL,
    valid_to TIMESTAMPTZ,
    change_reason TEXT,
    UNIQUE(profile_id, version_number)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_candidate_profile_versions_profile_id ON candidate_profile_versions(profile_id);
CREATE INDEX IF NOT EXISTS idx_candidate_profile_versions_user_id ON candidate_profile_versions(user_id);
CREATE INDEX IF NOT EXISTS idx_candidate_profile_versions_valid_period ON candidate_profile_versions(profile_id, valid_from, valid_to);

-- 4. Add columns to existing tables (with existence checks)
DO $$ 
BEGIN
    -- Add admin_notes to users
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users') THEN
        ALTER TABLE users ADD COLUMN IF NOT EXISTS admin_notes TEXT;
        ALTER TABLE users ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;
        ALTER TABLE users ADD COLUMN IF NOT EXISTS deleted_by TEXT REFERENCES users(id);
        ALTER TABLE users ADD COLUMN IF NOT EXISTS deletion_reason TEXT;
    END IF;
    
    -- Add admin_notes to candidate_profiles
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'candidate_profiles') THEN
        ALTER TABLE candidate_profiles ADD COLUMN IF NOT EXISTS admin_notes TEXT;
        ALTER TABLE candidate_profiles ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;
        ALTER TABLE candidate_profiles ADD COLUMN IF NOT EXISTS deleted_by TEXT REFERENCES users(id);
        ALTER TABLE candidate_profiles ADD COLUMN IF NOT EXISTS deletion_reason TEXT;
    END IF;
    
    -- Add admin_notes to companies (only if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'companies') THEN
        ALTER TABLE companies ADD COLUMN IF NOT EXISTS admin_notes TEXT;
        ALTER TABLE companies ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;
        ALTER TABLE companies ADD COLUMN IF NOT EXISTS deleted_by TEXT REFERENCES users(id);
        ALTER TABLE companies ADD COLUMN IF NOT EXISTS deletion_reason TEXT;
    END IF;
END $$;

-- 5. Indexes for soft delete (with existence checks)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users') THEN
        CREATE INDEX IF NOT EXISTS idx_users_deleted_at ON users(deleted_at) WHERE deleted_at IS NULL;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'candidate_profiles') THEN
        CREATE INDEX IF NOT EXISTS idx_candidate_profiles_deleted_at ON candidate_profiles(deleted_at) WHERE deleted_at IS NULL;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'companies') THEN
        CREATE INDEX IF NOT EXISTS idx_companies_deleted_at ON companies(deleted_at) WHERE deleted_at IS NULL;
    END IF;
END $$;

-- 6. Audit logging trigger function
CREATE OR REPLACE FUNCTION audit_user_changes() RETURNS TRIGGER AS $$
DECLARE
    audit_action VARCHAR(50);
    admin_user_id TEXT;
    old_record JSONB;
    new_record JSONB;
    changes JSONB;
    record_id_value TEXT;
BEGIN
    -- Determine the action
    IF TG_OP = 'DELETE' THEN
        audit_action := 'DELETE';
        old_record := to_jsonb(OLD);
        new_record := NULL;
        record_id_value := OLD.id::TEXT;
    ELSIF TG_OP = 'INSERT' THEN
        audit_action := 'CREATE';
        old_record := NULL;
        new_record := to_jsonb(NEW);
        record_id_value := NEW.id::TEXT;
    ELSIF TG_OP = 'UPDATE' THEN
        audit_action := 'UPDATE';
        old_record := to_jsonb(OLD);
        new_record := to_jsonb(NEW);
        record_id_value := NEW.id::TEXT;
        
        -- Check for specific action types
        IF OLD.role IS DISTINCT FROM NEW.role THEN
            audit_action := 'ROLE_CHANGE';
        ELSIF OLD.is_active IS DISTINCT FROM NEW.is_active THEN
            audit_action := 'STATUS_CHANGE';
        ELSIF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
            audit_action := 'SOFT_DELETE';
        ELSIF OLD.deleted_at IS NOT NULL AND NEW.deleted_at IS NULL THEN
            audit_action := 'RESTORE';
        END IF;
    END IF;
    
    -- Get the admin user ID from the current session
    admin_user_id := current_setting('app.current_user_id', true);
    
    -- Calculate changes for updates
    IF TG_OP = 'UPDATE' THEN
        SELECT jsonb_object_agg(key, jsonb_build_object('old', old_value, 'new', new_value))
        INTO changes
        FROM (
            SELECT key, old_record->key as old_value, new_record->key as new_value
            FROM jsonb_object_keys(old_record) AS key
            WHERE old_record->key IS DISTINCT FROM new_record->key
        ) AS diff;
    END IF;
    
    -- Get user_id for non-users tables
    DECLARE
        target_user_id TEXT;
    BEGIN
        IF TG_TABLE_NAME = 'users' THEN
            target_user_id := record_id_value;
        ELSIF TG_TABLE_NAME = 'candidate_profiles' THEN
            target_user_id := COALESCE(NEW.user_id, OLD.user_id);
        ELSIF TG_TABLE_NAME = 'companies' THEN
            target_user_id := record_id_value;
        ELSE
            target_user_id := record_id_value;
        END IF;
        
        -- Insert audit log
        INSERT INTO user_audit_logs (
            user_id,
            admin_id,
            action,
            table_name,
            record_id,
            old_data,
            new_data,
            changes,
            metadata
        ) VALUES (
            target_user_id,
            admin_user_id,
            audit_action,
            TG_TABLE_NAME,
            record_id_value,
            old_record,
            new_record,
            changes,
            jsonb_build_object(
                'timestamp', NOW(),
                'schema', TG_TABLE_SCHEMA,
                'operation', TG_OP
            )
        );
    END;
    
    IF TG_OP = 'DELETE' THEN
        RETURN OLD;
    ELSE
        RETURN NEW;
    END IF;
END;
$$ LANGUAGE plpgsql;

-- 7. User versioning trigger function
CREATE OR REPLACE FUNCTION version_user_changes() RETURNS TRIGGER AS $$
DECLARE
    new_version_number INTEGER;
    admin_user_id TEXT;
BEGIN
    IF TG_OP != 'UPDATE' THEN
        RETURN NEW;
    END IF;
    
    admin_user_id := current_setting('app.current_user_id', true);
    
    SELECT COALESCE(MAX(version_number), 0) + 1
    INTO new_version_number
    FROM user_versions
    WHERE user_id = NEW.id;
    
    UPDATE user_versions
    SET valid_to = NOW()
    WHERE user_id = NEW.id
    AND valid_to IS NULL;
    
    INSERT INTO user_versions (
        user_id,
        version_number,
        email,
        first_name,
        last_name,
        role,
        is_active,
        image_url,
        public_metadata,
        private_metadata,
        created_by,
        created_at,
        valid_from,
        change_reason
    ) VALUES (
        NEW.id,
        new_version_number,
        NEW.email,
        NEW.first_name,
        NEW.last_name,
        NEW.role,
        NEW.is_active,
        NEW.image_url,
        NEW.public_metadata,
        NEW.private_metadata,
        admin_user_id,
        NOW(),
        NOW(),
        current_setting('app.change_reason', true)
    );
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 8. Candidate profile versioning trigger function
CREATE OR REPLACE FUNCTION version_candidate_profile_changes() RETURNS TRIGGER AS $$
DECLARE
    new_version_number INTEGER;
    admin_user_id TEXT;
BEGIN
    IF TG_OP != 'UPDATE' THEN
        RETURN NEW;
    END IF;
    
    admin_user_id := current_setting('app.current_user_id', true);
    
    SELECT COALESCE(MAX(version_number), 0) + 1
    INTO new_version_number
    FROM candidate_profile_versions
    WHERE profile_id = NEW.id;
    
    UPDATE candidate_profile_versions
    SET valid_to = NOW()
    WHERE profile_id = NEW.id
    AND valid_to IS NULL;
    
    INSERT INTO candidate_profile_versions (
        profile_id,
        user_id,
        version_number,
        title,
        summary,
        experience_level,
        location,
        remote_preference,
        availability,
        salary_min,
        salary_max,
        salary_currency,
        linkedin_url,
        github_url,
        portfolio_url,
        resume_url,
        is_anonymized,
        is_active,
        public_metadata,
        private_metadata,
        created_by,
        created_at,
        valid_from,
        change_reason
    ) VALUES (
        NEW.id,
        NEW.user_id,
        new_version_number,
        NEW.title,
        NEW.summary,
        NEW.experience_level,
        NEW.location,
        NEW.remote_preference,
        NEW.availability,
        NEW.salary_min,
        NEW.salary_max,
        NEW.salary_currency,
        NEW.linkedin_url,
        NEW.github_url,
        NEW.portfolio_url,
        NEW.resume_url,
        NEW.is_anonymized,
        NEW.is_active,
        NEW.public_metadata,
        NEW.private_metadata,
        admin_user_id,
        NOW(),
        NOW(),
        current_setting('app.change_reason', true)
    );
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 9. Apply triggers (with existence checks)
DO $$
BEGIN
    -- Users table triggers
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users') THEN
        DROP TRIGGER IF EXISTS audit_users_changes ON users;
        CREATE TRIGGER audit_users_changes
            AFTER INSERT OR UPDATE OR DELETE ON users
            FOR EACH ROW EXECUTE FUNCTION audit_user_changes();
            
        DROP TRIGGER IF EXISTS version_users_changes ON users;
        CREATE TRIGGER version_users_changes
            AFTER UPDATE ON users
            FOR EACH ROW EXECUTE FUNCTION version_user_changes();
    END IF;
    
    -- Candidate profiles triggers
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'candidate_profiles') THEN
        DROP TRIGGER IF EXISTS audit_candidate_profiles_changes ON candidate_profiles;
        CREATE TRIGGER audit_candidate_profiles_changes
            AFTER INSERT OR UPDATE OR DELETE ON candidate_profiles
            FOR EACH ROW EXECUTE FUNCTION audit_user_changes();
            
        DROP TRIGGER IF EXISTS version_candidate_profiles_changes ON candidate_profiles;
        CREATE TRIGGER version_candidate_profiles_changes
            AFTER UPDATE ON candidate_profiles
            FOR EACH ROW EXECUTE FUNCTION version_candidate_profile_changes();
    END IF;
    
    -- Companies triggers (only if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'companies') THEN
        DROP TRIGGER IF EXISTS audit_companies_changes ON companies;
        CREATE TRIGGER audit_companies_changes
            AFTER INSERT OR UPDATE OR DELETE ON companies
            FOR EACH ROW EXECUTE FUNCTION audit_user_changes();
    END IF;
END $$;

-- 10. RLS policies
ALTER TABLE user_audit_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE candidate_profile_versions ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Admins can read audit logs" ON user_audit_logs;
DROP POLICY IF EXISTS "Admins can read all user versions" ON user_versions;
DROP POLICY IF EXISTS "Users can read their own versions" ON user_versions;
DROP POLICY IF EXISTS "Admins can read all profile versions" ON candidate_profile_versions;
DROP POLICY IF EXISTS "Users can read their own profile versions" ON candidate_profile_versions;

-- Create policies
CREATE POLICY "Admins can read audit logs"
    ON user_audit_logs FOR SELECT
    USING (auth.jwt() ->> 'role' = 'admin');

CREATE POLICY "Admins can read all user versions"
    ON user_versions FOR SELECT
    USING (auth.jwt() ->> 'role' = 'admin');

CREATE POLICY "Users can read their own versions"
    ON user_versions FOR SELECT
    USING (auth.uid()::text = user_id);

CREATE POLICY "Admins can read all profile versions"
    ON candidate_profile_versions FOR SELECT
    USING (auth.jwt() ->> 'role' = 'admin');

CREATE POLICY "Users can read their own profile versions"
    ON candidate_profile_versions FOR SELECT
    USING (auth.uid()::text = user_id);

-- Views for soft deletes (with existence checks)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users') THEN
        CREATE OR REPLACE VIEW active_users AS
        SELECT * FROM users WHERE deleted_at IS NULL;
        GRANT SELECT ON active_users TO authenticated;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'candidate_profiles') THEN
        CREATE OR REPLACE VIEW active_candidate_profiles AS
        SELECT * FROM candidate_profiles WHERE deleted_at IS NULL;
        GRANT SELECT ON active_candidate_profiles TO authenticated;
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'companies') THEN
        CREATE OR REPLACE VIEW active_companies AS
        SELECT * FROM companies WHERE deleted_at IS NULL;
        GRANT SELECT ON active_companies TO authenticated;
    END IF;
END $$;

-- Grant permissions
GRANT SELECT ON user_audit_logs TO authenticated;
GRANT SELECT ON user_versions TO authenticated;
GRANT SELECT ON candidate_profile_versions TO authenticated;

-- Verification
SELECT 'Migration completed!' as status;
SELECT 'Tables created:' as status, 
    EXISTS(SELECT FROM information_schema.tables WHERE table_name = 'user_audit_logs') as user_audit_logs,
    EXISTS(SELECT FROM information_schema.tables WHERE table_name = 'user_versions') as user_versions,
    EXISTS(SELECT FROM information_schema.tables WHERE table_name = 'candidate_profile_versions') as candidate_profile_versions;