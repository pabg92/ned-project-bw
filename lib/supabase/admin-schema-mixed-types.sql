-- Admin functionality database schema extensions (MIXED TYPES)
-- Handles: users.id = TEXT, candidate_profiles.id = UUID
...
CREATE TABLE IF NOT EXISTS user_audit_logs (
    ...
    record_id TEXT NOT NULL, -- TEXT to handle both UUID and TEXT ids
    old_data JSONB,
    new_data JSONB,
    changes JSONB,
...
-- 3. Candidate Profile Versions Table (UUID for profile_id)
CREATE TABLE IF NOT EXISTS candidate_profile_versions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    profile_id UUID NOT NULL REFERENCES candidate_profiles(id), -- UUID type
    user_id TEXT NOT NULL REFERENCES users(id), -- TEXT type
    version_number INTEGER NOT NULL,
    title VARCHAR(255),
    summary TEXT,
...
BEGIN
    ...
    BEGIN
        ...
        ELSIF TG_TABLE_NAME = 'companies' THEN
            -- For companies, the ID might be the user ID
            target_user_id := record_id_value;
        ELSE
            target_user_id := record_id_value;
        END IF;
